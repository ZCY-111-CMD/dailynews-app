...
import { SpecialTopic as SpecialTopicType } from '../types'
...
const SpecialTopicDetail = () => {
  ...
  const parsed = useMemo(() => {
    ...
    if (!rawContent && topic?.contentMarkdown) {
      rawContent = marked(topic.contentMarkdown) as string
    }
    if (!rawContent) return { html: '', toc: [] as Array<{ id: string; text: string; level: number }> }
    return parseArticle(rawContent)
  }, [topic?.content, topic?.contentMarkdown])
  ...
  const loadTopic = async (slug: string) => {
    try {
      const response: any = await api.specialTopics.getBySlug(slug)
      setTopic(response.data)
      setDisplayFavoriteCount(response.data.displayFavoriteCount || 0)
      const token = localStorage.getItem('user_token')
      if (token) {
        try {
          const favRes: any = await api.specialTopics.getFavorites({ limit: 1000 })
          const ids = (favRes.data || []).map((f: any) => f.id)
          setFavorited(ids.includes(response.data.id))
        } catch {}
      }
    } catch (err) {
      console.error('Failed to load special topic:', err)
      setError('加载失败，请稍后重试')
    } finally {
      setLoading(false)
    }
  ...
  const handleToggleFavorite = async (e: React.MouseEvent) => {
    ...
    try {
      if (favorited) {
        await api.specialTopics.unfavorite(topic.id)
        setFavorited(false)
        setDisplayFavoriteCount(prev => prev - 1)
      } else {
        await api.specialTopics.favorite(topic.id)
        setFavorited(true)
        setDisplayFavoriteCount(prev => prev + 1)
      }
    } catch (error) {
  ...
  if (error || !topic) {
    return (
      <div className="text-center py-16">
        <p className="text-slate-600">{error || '专题未找到'}</p>
        <button
      ...
      <article ref={articleRef} className="mx-auto max-w-wide py-10">
        <div className="grid grid-cols-12 gap-8">
          <div className="col-span-12 xl:col-span-8">
                ...
                <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-sm text-white/85">专题栏目</span>
                    </div>
                    <button
                  ...
                  <p className="mt-4 max-w-2xl text-base sm:text-lg text-white/85 line-clamp-3">
                    {topic.description}
                  </p>
            ...
            {topic.author && (
              <div className="reveal-item mt-6 flex items-center gap-3 p-5 bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl" data-reveal-delay="100">
                ...
                <div>
                  ...
                  <div className="text-lg font-semibold text-slate-900">{topic.author}</div>
                </div>
              </div>
            )}
...
export default SpecialTopicDetail
